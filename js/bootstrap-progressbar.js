/*
 * Modified version of original found at: http://geersch.github.io/bootstrap-progressbar/
 * Modified by Andrew Kelly
 * The original was modified so that the progress bar went from danger (red) to
 *  success (green) instead of from success to danger.
 */
!function ($) {
	...
	ProgressBar.prototype = {
		...
		setWarningMarker: function (marker) {
			...
			if (marker > this.successMarker) {
				this.warningMarker = this.successMarker;
				return;
			}
		...
		setPosition: function (position) {
		    ...
		    try {
			    ...
			    this.element.find('.bar-danger').css('width', this.warningMarker + "%");
			    if (this.percent > this.warningMarker && this.percent <= this.successMarker) {
				    this.element.find('.bar-warning').css('width', (this.percent - this.warningMarker) + "%");
				    this.element.find('.bar-success').css('width', "0%");
				    return;
			    }

			    this.element.find('.bar-warning').css('width', (this.successMarker - this.warningMarker) + "%");
